...
function App() {
  ...
  const [viewingData, setViewingData] = useState(null); // State for Data Viewer Modal (Fixed RefError via HMR)
      ...
      >
        <div className="content-wrapper dashboard-content">
          ...
          <div className={`dashboard-card ${archiveData.chaosMode ? 'chaos-border' : ''}`}>
            ...
            {/* History Dropdown (Silver/Black) */}
            {archiveData.history.length > 0 && (
              <div style={{ marginTop: '10px', textAlign: 'center' }}>
                <select
                  onChange={(e) => {
                    const selectedTx = e.target.value;
                    if (!selectedTx) return;
                    const record = archiveData.history.find(h => h.txId === selectedTx);
                    if (record) {
                      setViewingData({
                        title: `Archive Record (${record.date})`,
                        content: record,
                        txId: record.txId
                      });
                    }
          ...
          {isLocalEnvironment() && (
            <div style={{
              ...
              <strong style={{ color: '#ccc', display: 'block', marginBottom: '15px', fontSize: '0.8rem' }}>ADMIN: SYSTEM CONTROLS (Updated)</strong>
